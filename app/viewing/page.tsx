...
function RoomPreview({ room, onOpen }: { room: Room; onOpen: (room: Room) => void }) {
    ...
    return (
        <article role="button" tabIndex={0} onClick={() => onOpen(room)} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") onOpen(room); }} className="group cursor-pointer overflow-hidden border border-[#dcd9d1] bg-[#fbfaf7] transition-transform duration-300 hover:-translate-y-1 focus:outline-none focus:ring-2 focus:ring-[#397052]">
        ...
        <main className="min-h-screen bg-[#f4f2ed] text-[#202522]">
            ...
            {selectedRoom && <div className="fixed inset-0 z-50 flex items-end justify-center bg-[#202522]/60 p-0 sm:items-center sm:p-6" role="presentation" onClick={closeDetails}>
                <section role="dialog" aria-modal="true" aria-labelledby="room-detail-title" onClick={(event) => event.stopPropagation()} className="max-h-[92vh] w-full max-w-4xl overflow-y-auto bg-[#fbfaf7] shadow-2xl">
                        <div className="flex items-center justify-between border-b border-[#dcd9d1] px-5 py-4 sm:px-7"><p className="text-xs font-bold uppercase tracking-[0.18em] text-[#397052]">Room details</p><button type="button" onClick={closeDetails} aria-label="Close room details" className="p-1 text-[#707770] hover:text-[#202522]"><XIcon size={22} /></button></div>
